...
def prediction_tr(match_pk, username):
    match = Match.objects.filter(pk=match_pk).values(
        'pk', 'team1__team_name', 'team1__short_name', 'team1_score', 'team2__team_name', 'team2__short_name', 'team2_score', 'best_of', 'region', 'start_timestamp', 'elo_processed').first()
    context = {'match': match}
